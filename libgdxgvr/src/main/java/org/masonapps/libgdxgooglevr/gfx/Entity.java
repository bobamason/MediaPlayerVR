...
public class Entity implements Disposable {
    ...
    public boolean intersectsRaySphere(Ray ray, @Nullable Vector3 hitPoint) {
        return intersectsRaySphere(ray, hitPoint, false);
    }

    public boolean intersectsRaySphere(Ray ray, @Nullable Vector3 hitPoint, boolean insideBounds) {
        if (!updated) recalculateTransform();
        final Ray tmpRay = Pools.obtain(Ray.class).set(ray);
        final Vector3 tmp = Pools.obtain(Vector3.class);
//        tmpRay.mul(inverseTransform);
        final boolean intersectRaySphere = Intersector.intersectRaySphere(tmpRay, tmp.set(center).add(position), (insideBounds ? Math.min(dimensions.x, Math.min(dimensions.y, dimensions.z)) / 2f : radius) * Math.min(scale.x, Math.min(scale.y, scale.z)), hitPoint);
