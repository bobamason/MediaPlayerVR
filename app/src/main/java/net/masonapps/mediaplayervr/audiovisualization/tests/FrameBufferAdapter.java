...
/**
 * Created by Bob on 3/17/2017.
 */

public class FrameBufferAdapter extends BaseAppAdapter {
    ...
    protected void onCreate() {
        ...
        enviroment.set(ColorAttribute.createAmbient(Color.GRAY));
        ...
        for (int i = 0; i < colors.length; i++) {
            instances.add(new ModelInstance(createSphere(modelBuilder, colors[i]), 0, 0, -5));
        }
    ...
    protected ShaderProvider createShaderProvider() {
        return new PhongShaderProvider();
    }

    private Model createSphere(ModelBuilder modelBuilder, Color c) {
        ...
        return modelBuilder.createSphere(1f, 1f, 1f, 24, 16, material, VertexAttributes.Usage.Position | VertexAttributes.Usage.Normal);
    }
...
//        spriteBatch.setColor(color.set(0, 0, 0, 0.05f));
        ...
        spriteBatch.draw(fbo.getColorBufferTexture(), -fbo.getWidth() / 2 + 5, -fbo.getHeight() / 2 + 5, fbo.getWidth() - 10, fbo.getHeight() - 10, 0, 0, fbo.getWidth(), fbo.getHeight(), false, true);
